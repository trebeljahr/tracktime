...
`ios/` is **committed** — the bundle identifier, the display name, the ATS
exception, the orientation set and (later) entitlements are all hand-edits that
live only in that tree, and `cap add` writes none of them. Two things inside it
are generated and still tracked: `App/CapApp-SPM/Package.swift`, rewritten by
every `cap sync` (a diff there after a build is normal — commit it when the
plugin set changed), and the `Assets.xcassets` catalog, regenerated by
`pnpm mobile:assets` from `resources/`. `android/` is still gitignored; the
Android stage un-ignores it the same way.
...
`capacitor://localhost` and `https://localhost` are trusted by `scripts/dev.mjs`
directly — not via `.env.development`, whose value dotenvx skips because the dev
